...
sl_status_t send_data_distance(uint8_t connection,
...
//  echo_time_out = false;
//  app_log("distance: %d\n", distance);
  send_data_distance(connection_handle, gattdb_data_distance, distance_test);
}
...
sl_status_t send_data_distance(uint8_t connection,
    ...
    // In ra giá trị của connection và characteristic để kiểm tra
    app_log("Connection: %u, Characteristic: %u\n", connection, characteristic);
    ...
    if (sc == SL_STATUS_INVALID_HANDLE) {
        app_log("Invalid handle: Connection or Characteristic is incorrect.\n");
    } else if (sc != SL_STATUS_OK) {
        app_log("Indication failed with status: 0x%04x\n", sc);
    } else {
        app_log("Indication succeeded: %s\n", data_send);
    }
